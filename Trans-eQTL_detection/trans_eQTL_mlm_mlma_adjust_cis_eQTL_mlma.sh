...
if [ -e ./fixed/${gene_list[i]}.fixed.txt ]; then
gcta64 --mlma --bfile ${tissue}_GRM_input --grm ${tissue}_geno_grm --autosome-num 29 \
--pheno phenotype.txt --mpheno ${pheno_id}  --qcovar cvrt.txt --covar ./fixed/${gene_list[i]}.fixed.txt  --reml-maxit 2000 --reml-bendV \
--thread-num 10 --out output_mlm_corrected/${gene_list[i]}.assoc
if [ $? -gt 0 ]; then
...
awk -v gene=${gene_list[i]} '{if(NR==1)print "Gene""\t"$0; else if($9<0.00001)print gene"\t"$0}' output_mlm_corrected/${gene_list[i]}.assoc.mlma > output_mlm_corrected/${gene_list[i]}.assoc.mlma.tmp
rm output_mlm_corrected/${gene_list[i]}.assoc.mlma
done

...
d=read.table(gzfile(paste0(tissue,".permutations.2rd.txt.gz")),hea=F,stringsAsFactors=F)
...
d$st=p.adjust(d$bpval,method="BH")
write.table(d[which(d$st <= 0.05), ], paste0(tissue,".permutations.2rd.storey.txt"), quote=F, row.names=F, col.names=T)
